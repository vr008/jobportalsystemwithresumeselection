...
function DisplayJobs() {
  ...
  const handleJobDelete = async (jobId) => {
    try {
      ...
      await axios.delete(`http://localhost:4000/admin/deletejob/${jobId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      // After successful deletion, fetch the updated job list from the server
      const response = await axios.get('http://localhost:4000/auth/showjobs');
      setJobs(response.data.jobs);
    } catch (error) {
      ...
      <nav className="navbar navbar-expand-lg bg-white navbar-light shadow sticky-top p-0">
        ...
        <div className="collapse navbar-collapse" id="navbarCollapse">
          ...
          <Link to="/adminhome" className="nav-item nav-link">Home</Link>
             ...
             <Link to="/delete" className="nav-item nav-link  active">deletejob</Link>
                    ...
                    <div className="item-overlay">
                      <div className="job-info">
                        ...
                        <ul className="job-social tr-list">
                         ...
                         <center> <button className="btn btn-danger" onClick={() => handleJobDelete(job._id)}>
                          Delete
                        </button></center>
